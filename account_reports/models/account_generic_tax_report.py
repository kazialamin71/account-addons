...
from .account_report import query_to_sql_parts
...
class GenericTaxReportCustomHandler(models.AbstractModel):
    ...
    def _read_generic_tax_report_amounts_no_tax_details(self, report, options, options_by_column_group):
        ...
        company_query = self.env['account.tax'].with_context(active_test=False)._search(company_domain)
        _tables, company_where_clause, company_where_params = query_to_sql_parts(company_query)
        self._cr.execute(
            f'''
                SELECT
                    account_tax.id,
    ...
    def _build_report_line(self, report, options, default_vals, groupby_key, value, parent_line_id):
        ...
        if groupby_key == 'src_tax_type_tax_use':
            ...
            if options.get('multi-company'):
                company_names = ' / '.join(account.company_ids.mapped('display_name'))
                report_line['name'] = f"{account.display_name} - {company_names}"
            else:
