...
class AgedPartnerBalanceCustomHandler(models.AbstractModel):
    ...
    def _custom_line_postprocessor(self, report, options, lines, warnings=None):
        ...
        if partner_lines_map:
            # Since Odoo 19, company-dependent values are stored directly on
            # the model as JSONB instead of in the removed ir_property table.
            partners = self.env['res.partner'].browse(partner_lines_map).with_company(report.env.company)
            trust_map = {partner.id: partner.trust or 'normal' for partner in partners}
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        ...
        query = f"""
            ...
            SELECT
                ...
                ARRAY_AGG(DISTINCT account.code_store->>SPLIT_PART(company.parent_path, '/', 1)) AS account_name,
                ARRAY_AGG(DISTINCT COALESCE(account_move_line.date_maturity, account_move_line.date)) AS due_date,
                ARRAY_AGG(DISTINCT account_move_line.currency_id) AS currency_id,
                COUNT(account_move_line.id) AS aml_count,
                ARRAY_AGG(account.code_store->>SPLIT_PART(company.parent_path, '/', 1)) AS account_code,
                {select_period_query}
            ...
            JOIN res_company company ON company.id = account_move_line.company_id
