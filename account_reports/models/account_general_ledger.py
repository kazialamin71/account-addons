    ...
    def _query_values(self, report, options):
        ...
        if groupby_companies:
            ...
            for account in self.env['account.account'].browse(candidates_account_ids):
                # Accounts can be shared by several companies in Odoo 19.
                # Allocate every pending company's current-year earnings to
                # the shared unaffected-earnings account.
                for company in account.company_ids:
                    company_unaffected_earnings = groupby_companies.pop(company.id, None)
                    if not company_unaffected_earnings:
                        continue
                    account_values = groupby_accounts.setdefault(
                        account.id,
                    ...
                    for column_group_key in options['column_groups']:
                        ...
                        current = account_values[column_group_key].get('unaffected_earnings')
                        if current:
                            for key in ('debit', 'credit', 'balance'):
                                current[key] = current.get(key, 0.0) + unaffected_earnings.get(key, 0.0)
                        else:
                            account_values[column_group_key]['unaffected_earnings'] = unaffected_earnings
        ...
        for column_group_key, group_options in report._split_options_per_column_group(options).items():
            ...
            query = f'''
                (SELECT
                    ...
                    account.code_store->>SPLIT_PART(company.parent_path, '/', 1) AS account_code,
