        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            queries.append(f'''
                SELECT
                    ...
                    account_account.code_store->>SPLIT_PART(company.parent_path, '/', 1) AS account_code,
    ...
    def _get_liquidity_moves(self, report, options, currency_table_query, payment_account_ids, cash_flow_tag_ids):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            queries.append(f'''
                ...
                GROUP BY account_move_line.company_id, account_move_line.account_id, account_code, account_name, account_account.account_type, account_account_account_tag.account_account_tag_id
                ...
                WHERE account_move_line.move_id IN (SELECT unnest(payment_move_ids.move_id) FROM payment_move_ids)
                    AND account_move_line.account_id NOT IN %s
                GROUP BY account_move_line.account_id, account_code, account_name, account_account.account_type, account_account_account_tag.account_account_tag_id)
            ''')
        ...
        for column in options['columns']:
            queries.append(f'''
                ...
                LEFT JOIN account_account_account_tag
                    ...
                    AND account_account_account_tag.account_account_tag_id IN %s
                WHERE account_move_line.move_id IN %s
                GROUP BY account_move_line.move_id, account_move_line.account_id, account_code, account_name, account_account.account_type, account_account_account_tag.account_account_tag_id
            ''')
