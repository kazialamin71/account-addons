...
def query_to_sql_parts(query):
    """Return the legacy ``(tables, where, params)`` shape from a v19 Query.

    The report engine still builds a number of carefully tuned SQL statements
    around ORM domains.  Odoo 19 exposes those parts as composable SQL objects
    instead of ``Query.get_sql()``.
    """
    from_clause = query.from_clause
    where_clause = query.where_clause
    return (
        from_clause.code,
        where_clause.code or 'TRUE',
        [*from_clause.params, *where_clause.params],
    )
...
DATE_FILTER_ALIASES = {
    ...
    'this_return_period': 'this_tax_period',
    'previous_return_period': 'last_tax_period',
}
...
class AccountReport(models.Model):
    ...
    # Preserve the date filter keys consumed by this report engine and its
    # OWL filters.  Odoo 19 renamed these values on the lightweight base
    # model, but changing the keys here would break saved report options.
    default_opening_date_filter = fields.Selection(
        ...
        ondelete={
            'last_month': 'set null',
            'last_quarter': 'set null',
            'last_year': 'set null',
            'this_tax_period': 'set null',
            'last_tax_period': 'set null',
        },
    )

    # Full reports can switch between the domestic tax position and foreign
    # VAT registrations.  This option was removed from Odoo 19's base model,
    # while the report engine still needs it.
    filter_fiscal_position = fields.Boolean(
        string='Filter Multi-VAT',
        compute=lambda report: report._compute_report_option_filter('filter_fiscal_position'),
        readonly=False,
        store=True,
    ...
    def _init_options_date(self, options, previous_options=None):
        ...
        previous_filter = DATE_FILTER_ALIASES.get(
            previous_date.get('filter', 'custom'),
            previous_date.get('filter', 'custom'),
        )

        options_mode = 'range' if self.filter_date_range else 'single'
        default_filter = DATE_FILTER_ALIASES.get(
            self.default_opening_date_filter,
            self.default_opening_date_filter,
        ) or ('this_year' if options_mode == 'range' else 'today')
        date_from = date_to = period_type = False

        if previous_mode == 'single' and options_mode == 'range':
            # 'single' date mode to 'range'.
            if previous_filter and (previous_date_to or previous_date_from):
            ...
            if previous_filter == 'custom' and (previous_date_to or previous_date_from):
                date_to = fields.Date.from_string(previous_date_to or previous_date_from)
            ...
            if previous_filter == 'custom':
                if options_mode == 'range':
                    ...
                    date_to = fields.Date.from_string(previous_date_to or previous_date_from)
                    date_from = date_to and date_utils.get_month(date_to)[0]
        ...
        # Discard incomplete custom dates saved by an older client instead of
        # passing False to the fiscal-year date helpers.
        if options_filter == 'custom' and (not date_to or (options_mode == 'range' and not date_from)):
            options_filter = default_filter
    ...
    def _get_options_fiscal_position_domain(self, options):
        ...
        if fiscal_position_opt == 'domestic':
            ...
            return Domain.OR([domain, tax_tag_domain])
    ...
    def _query_get(self, options, date_scope, domain=None):
        ...
        # Odoo 19's _search() compiles the domain and applies access rules.
        query = self.env['account.move.line']._search(domain)

        return query_to_sql_parts(query)
    ...
    def _compute_formula_batch_with_engine_account_codes(self, options, date_scope, formulas_dict, current_groupby, next_groupby, offset=0, limit=None, warnings=None):
        ...
        for prefix, excluded_prefixes in prefixes_to_compute:
            ...
            if excluded_prefixes_domains:
                ...
                account_domain += Domain.OR(excluded_prefixes_domains)

            prefix_tables, prefix_where_clause, prefix_where_params = query_to_sql_parts(
                self.env['account.account']._search(account_domain)
            )
    ...
    def _compute_formula_batch_with_engine_external(self, options, date_scope, formulas_dict, current_groupby, next_groupby, offset=0, limit=None, warnings=None):
        ...
        # Do the computation
        _tables, where_clause, where_params = query_to_sql_parts(
            self.env['account.report.external.value']._search(external_value_domain)
        )
        currency_table_query = self._get_query_currency_table(options)
    ...
    def action_audit_cell(self, options, params):
        ...
        if expression.engine == 'external':
            ...
            if expression.formula == 'most_recent':
                tables, where_clause, where_params = query_to_sql_parts(
                    self.env['account.report.external.value']._search(external_values_domain)
                )
                self._cr.execute(f"""
                    SELECT ARRAY_AGG(id)
                    FROM {tables}
    ...
    def _get_audit_line_domain(self, column_group_options, expression, params):
        ...
        for expression_to_audit in expression._expand_aggregations():
            ...
            audit_or_domains.append(Domain.AND([
                expression_domain,
                groupby_domain,
            ]))
    ...
    def _get_query_currency_table(self, options):
        ...
        query = '(VALUES %s) AS currency_table(company_id, rate, precision)' % ','.join(
            '(%s, %s, %s)' for _company in companies
        )
...
class AccountReportExpression(models.Model):
    ...
    # Odoo 19's lightweight account report model no longer exposes the two
    # scopes used by the full financial report engine.  Keep them on the
    # inherited field: ``normal`` includes opening balances according to the
    # account type, while ``previous_tax_period`` powers tax carryovers.
    date_scope = fields.Selection(
        ...
        ondelete={
            'normal': 'set default',
            'previous_tax_period': 'set default',
        },
    )

    def _expand_aggregations(self):
        """Expand aggregation dependencies, including legacy cross-reports.

        This engine intentionally supports formulas combining codes from more
        than one report (the Executive Summary is the notable case).  Odoo 19's
        base implementation restricts a cross-report expression to one explicit
        report; retain the full financial engine's multi-report behaviour.
        """
        result = self
        to_expand = self.filtered(lambda expression: expression.engine == 'aggregation')
        ...
        while to_expand:
            ...
            for candidate in to_expand:
                if candidate.formula == 'sum_children':
                    sub_expressions |= candidate.report_line_id.children_ids.expression_ids.filtered(
                        lambda expression: expression.label == candidate.label
                    )
                    continue

                labels_by_code = candidate._get_aggregation_terms_details()
                cross_report_domain = []
                if candidate.subformula != 'cross_report':
                    cross_report_domain = [
                        ('report_line_id.report_id', '=', candidate.report_line_id.report_id.id),
                    ]

                for line_code, expression_labels in labels_by_code.items():
                    domains.append([
                        ('report_line_id.code', '=', line_code),
                        ('label', 'in', tuple(expression_labels)),
                        *cross_report_domain,
                    ])
            ...
            to_expand = sub_expressions.filtered(
                lambda expression: expression.engine == 'aggregation' and expression not in result
            )
...
class AccountReportExternalValue(models.Model):
    ...
    # Required to isolate manual values and tax carryovers for foreign VAT
    # registrations.  The field belonged to the base model before Odoo 19.
