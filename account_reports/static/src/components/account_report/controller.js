...
export class AccountReportController {
    ...
    sessionOptionsID() {
        ...
        */
        // Odoo 19 moves company data from ``session.user_companies`` to the
        // user singleton and removes the former object from the session once
        // the web client is initialized.
        const companyId = user.activeCompany?.id ?? user.defaultCompany?.id ?? user.context.allowed_company_ids?.[0] ?? "none";
        return `account.report:${ this.actionReportId }:${ companyId }`;
    }
